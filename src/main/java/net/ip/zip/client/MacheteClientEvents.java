...
public class MacheteClientEvents {
    ...
    private static final Set<String> ONE_SHOT = Set.of(
            "machete_hit", "machete_alt_hit", "machete_heavy_blow"
    );
    private static final int ATTACK_COOLDOWN = 12;
    private static final int HEAVY_CHARGE_TICKS = 10;
    private static final String DATA_KEY = "machete_animation";
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if ("machete_heavy_blow".equals(curAnim)) {
        ...
        if (!attackDown && wasDown) {
            ...
            if (player.tickCount - lastTick >= ATTACK_COOLDOWN) {
                ...
                if (heavy) {
                    playOneShot(player, "machete_heavy_blow", true);
                } else {
                    int counter = hitCounter.getOrDefault(uuid, 0);
                    String anim = (counter % 2 == 0) ? "machete_hit" : "machete_alt_hit";
        ...
        String moveAnim;
        if (isSprinting) {
            moveAnim = "machete_sprinting";
        } else if (isMoving) {
            moveAnim = "machete_running";
        } else {
            moveAnim = "machete_idle";
        }
    ...
    public static void onInteractionKey(InputEvent.InteractionKeyMappingTriggered event) {
        Minecraft mc = Minecraft.getInstance();
        if (mc.player == null) return;
        if (mc.player.getMainHandItem().getItem() instanceof MacheteItem) {
            if (event.isAttack()) {
