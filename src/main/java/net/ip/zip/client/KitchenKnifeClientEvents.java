...
public class KitchenKnifeClientEvents {
    ...
    private static final Set<String> ONE_SHOT = Set.of(
            "kitchen_knife_hit", "kitchen_knife_hit2", "kitchen_knife_alt_hit"
    );
    private static final int ATTACK_COOLDOWN = 10;
    private static final int HEAVY_CHARGE_TICKS = 10;
    private static final String DATA_KEY = "kitchen_knife_animation";
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (!(stack.getItem() instanceof KitchenKnifeItem)) {
        ...
        if ("kitchen_knife_alt_hit".equals(curAnim)) {
        ...
        if (!attackDown && wasDown) {
            ...
            if (player.tickCount - lastTick >= ATTACK_COOLDOWN) {
                ...
                if (heavy) {
                    playOneShot(player, "kitchen_knife_alt_hit", true);
                } else {
                    int counter = hitCounter.getOrDefault(uuid, 0);
                    String anim = (counter % 2 == 0) ? "kitchen_knife_hit" : "kitchen_knife_hit2";
        ...
        String moveAnim;
        if (isSprinting) {
            moveAnim = "kitchen_knife_sprinting";
        } else if (isMoving) {
            moveAnim = "kitchen_knife_running";
        } else {
            moveAnim = "kitchen_knife_idle";
        }
    ...
    public static void onInteractionKey(InputEvent.InteractionKeyMappingTriggered event) {
        Minecraft mc = Minecraft.getInstance();
        if (mc.player == null) return;
        if (mc.player.getMainHandItem().getItem() instanceof KitchenKnifeItem) {
            if (event.isAttack()) {
