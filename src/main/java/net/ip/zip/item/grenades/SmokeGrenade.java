package net.ip.zip.item.grenades;

import net.ip.zip.entity.ModEntities;
import net.ip.zip.entity.SmokeGrenadeEntity;
import net.ip.zip.item.BaseThrowableItem;
import net.minecraft.world.entity.player.Player;
import net.minecraft.world.entity.projectile.ThrowableItemProjectile;
import net.minecraft.world.level.Level;

public class SmokeGrenade extends BaseThrowableItem {
    public SmokeGrenade(Properties properties) {
        super(properties, "smoke_grenade", 1.5f);
    }

    @Override
    protected ThrowableItemProjectile createEntity(Player player, Level level) {
        return new SmokeGrenadeEntity(ModEntities.SMOKE_GRENADE.get(), player, level);
    }
}